...
<div class="container">
    ...
    <ul class="nav nav-tabs">
        ...
        <li class="active"><a data-toggle="tab" href="#subjects">Enroll</a></li>
        <li><a data-toggle="tab" href="#pending" onclick="showpending();">Pending</a></li>
        <li><a data-toggle="tab" href="#approved" onclick="showapproved()">Approved</a></li>
    </ul>

    <div class="tab-content">

            <div id="subjects" class="tab-pane fade in active " style="background: white">
               <table class="table table-striped" id="enrolltable" style="max-height: 73%;overflow-y: auto">
                   <thead>
                       <tr>
             ...
             success:function(data)
             {
                 if(data['nosession'] === false)
                 {
                    if(data['success'] === true)
                    {
                        ...
                        {
                            list = list+"<li onclick='showsubjects("+sectionlist[i].id+")' style='cursor:pointer'><a>"+sectionlist[i].name+"</a></li>"
                        }
    ...
    $.ajax(
        {
            ...
            success:function(data)
            {
                ...
                {
                    tablerow = tablerow+"<tr><td><input name='secs[]' type='checkbox' value="+subs[i].id+"></td><td>"+subs[i].name+"</td><td>"+subs[i].code+"</td><td>"+subs[i].credit+"</td><td><select class='form-control' id="+subs[i].id+"><option value='0'>Regular</option><option value='1'>Retake</option><option value='2'>Recourse</option></select></td></tr>";
                }
                ...
                $.ajax(
                    {
                        url:'postenroll',
                        method:'post',
                        data:{subjects_id:checked_subjects_id,type:checked_subjects_id_type,section:section_name_global},
                        success:function(data)
                        {
                            if(data['nosession'] === false)
                            {
