...
<div class="container">
    ...
    <ul class="nav nav-tabs">
        <li id="checkall"><a><input type="checkbox" onchange="check()" id="checkinput"></a></li>
        <li class="dropdown">
        ...
        <li><a data-toggle="tab" href="#retake" onclick="getretakestudent()">Retake</a></li>
        <li><a data-toggle="tab" href="#update" onclick="getupdatestudent()">Update</a></li>
    </ul>
    <div class="tab-content">
        <div id="student" class="tab-pane fade in active" style="background: #fff">
                <div class="table-responsive" style="overflow-y:auto; max-height: 73%">
                    <table id="studenttabtable" class="table table-bordred table-striped">
                        <thead>
                        </thead>
                        <tbody id="studenttabtbody">  
                        </tbody>
                    </table>
                    <center> <button type="button" class="btn btn-success" id="upload" onclick="uploadresult()"><span class="glyphicon glyphicon-ok-sign"></span>&nbsp;Upload</button>
                </div>
        </div>
        ...
        <div id="retake" class="tab-pane fade" style="background: #fff">
            <div class="table-responsive" style="overflow-y:auto; max-height: 73%">
                ...
                <center> <button type="button" class="btn btn-success" id="retakeupload" onclick="retakeuploadresult()"><span class="glyphicon glyphicon-ok-sign"></span>&nbsp;Upload</button>
            </div>
        </div>
        <div id="update" class="tab-pane fade" style="background: #fff">
            <div class="table-responsive" style="overflow-y:auto; max-height: 73%">
                <table id="updatetabtable" class="table table-bordred table-striped">
                    <thead>
                    </thead>
           ...
           success:function(data)
           {
               if(data['error'] === false)
               {
                   if(data['nodata'] === false)
                   {
                        ...
                        {
                            list = list+"<li onclick='getstudents("+datas[i].sec_id+","+datas[i].sub_id+")'><a>"+datas[i].subname+' - ('+datas[i].secname+")</a></li>"; 
                        }
                ...
                if(data['user'] === true)
               {
                   ...
                   if(data['nodata'] === false)
                   {
                       ...
                       for(var i=0;i<datas.length;i++)
                       {
                           ...
                           tablerow = tablerow+ "<tr><td><input type='checkbox' name='id[]' value="+datas[i].id+"></td><td>"+datas[i].name+"</td><td>"+datas[i].student_id+"</td><td><input type='number' class='form-control' name='attendance[]' placeholder='Attendance Marks (0-10)'></td><td><input type='number' class='form-control' name='rora[]' placeholder='Report Or Assignment (0-10)'></td><td><input type='number' class='form-control' name='ct[]' placeholder='Class Test (0-10)'></td><td><input type='number' class='form-control' name='mid[]' placeholder='Mid Term (0-20)'></td><td><input type='number' class='form-control' name='final[]' placeholder='Final (0-50)'></td></tr>";
                       }
...
function uploadresult()
{
   ...
   for(var i=0;i<id.length;i++)
   {
       ...
       {
           //CHECKED
           if(parseFloat(temp_attndce[i].value)>=0 && parseFloat(temp_attndce[i].value)<=10 &&  parseFloat(temp_rora[i].value)>=0 && parseFloat(temp_rora[i].value)<=10 && parseFloat(temp_ct[i].value)>=0 && parseFloat(temp_ct[i].value)<=10 && parseFloat(temp_mid[i].value)>=0 && parseFloat(temp_mid[i].value)<=20 && parseFloat(temp_final[i].value)>=0 && parseFloat(temp_final[i].value)<=50 && temp_attndce[i].value != ' ' && temp_rora[i].value != ' ' && temp_ct[i].value != ' ' && temp_mid[i].value != ' ' && temp_final[i].value != ' ')
           {
        ...
        {   
            ...
            else
            {
                ...
                $.ajax(
                    ...
                    method:'post',
                    data:{id:checked_id,attendance:attendance,rora:rora,ct:ct,mid:mid,final:final,grade:grade,cgpa:cgpa,section:section_id_global,subject:subject_id_global},
                    success:function(data)
                    {
                        if(data['update'] === true)
                        {
...
function getretakestudent()
{
    ...
    $.ajax({
        ...
        method:'get',
        data:{section:section_id_global,subject:subject_id_global},
        success:function(data)
        {
            if(data['user'] === true)
               {
        ...
        {   
            ...
            else
            {
                ...
                $.ajax(
                    ...
                    method:'post',
                    data:{id:checked_id,attendance:attendance,rora:rora,ct:ct,mid:mid,final:final,grade:grade,cgpa:cgpa},
                    success:function(data)
                    {
                        console.log(data);
                        if(data['update'] === true)
                  ...
                  if(data['nodata'] === false)
                  {
                      if(data['regrec'].length > 0)
                      {
                          ...
                          for(var i=0;i<data['regrec'].length;i++)
                          {
                              ...
                              update_reg_rec = update_reg_rec+"<tr><td>"+datas[i].name+"</td><td>"+datas[i].student_id+"</td><td><input type='number' class='form-control' placeholder='Total ("+sum+")' readonly></td><td><input type='text' class='form-control' placeholder='Grade ("+datas[i].grade+")' readonly></td><td><input type='text' class='form-control' placeholder='CGPA ("+datas[i].cgpa+")' readonly></td><td><button class='btn btn-danger' onclick='regrec("+datas[i].id+")'>Restore</button></td></tr>";
                          }
                      }
                      else
                      {
