   ...
   $.ajax(
       {
           ...
           success:function(data)
           {
               ...
               if(data['hassection'] === true)
               {
                   ...
                   {
                       tablerow = tablerow + "<li onclick='showstudentlistforadvisor("+sections[i].id+")'><a>"+sections[i].name+"</a></li>"
                   }
               ...
               {
                var errors = "<div class='alert alert-danger'><strong></strong>&nbsp;Something Went Wrong.</div>";
...
function showstudentlistforadvisor(id)
{
    ...
    $.ajax({
        ...
        success:function(data)
        {
               ...
               if(datas[0].length>0)
                {
                    for(var i=0;i<datas.length;i++)
                    {
                        for(var j=0;j<datas[i].length;j++)
                        {
                            ...
                            tablerow = tablerow + "<tr><td>"+temp_data_holder[j].uname+"</td><td>"+temp_data_holder[j].student_id+"</td><td>"+temp_data_holder[j].subname+"</td><td>"+temp_data_holder[j].secname+"</td><td>"+retakeorrecourse(temp_data_holder[j].type)+"</td><td><button type='button' class='btn btn-success' onclick='acceptpending("+temp_data_holder[j].id+")'>Accept</button></td><td><button type='button' class='btn btn-danger' onclick='deletepending("+temp_data_holder[j].id+")'>Decline</button></td></tr>";
                        }
           ...
           {
               var errors = "<div class='alert alert-danger'><strong></strong>&nbsp;0 student enrolled for this section yet. </div>";
               document.getElementById('msg').innerHTML =errors;
               document.getElementById('msg').style.display='block';
               document.getElementById('pendingtable').style.display='none';
               var hide = setTimeout(hidemsg,4000);
                        ...
                        if(data['hasdata'] === true)
                        {
                            if(datas[0].length>0)
                            {
                                for(var i=0;i<datas.length;i++)
                                {
                                    ...
                                    {
                                        var temp_data_holder = datas[i];
                                        tablerow = tablerow + "<tr><td>"+temp_data_holder[j].uname+"</td><td>"+temp_data_holder[j].student_id+"</td><td>"+temp_data_holder[j].subname+"</td><td>"+temp_data_holder[j].secname+"</td><td>"+retakeorrecourse(temp_data_holder[j].type)+"</td><td><button type='button' class='btn btn-danger' onclick='makepending("+temp_data_holder[j].id+")'>Make Pending</button></td></tr>";
                                    }
